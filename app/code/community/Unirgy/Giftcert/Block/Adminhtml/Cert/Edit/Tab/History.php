...
class Unirgy_Giftcert_Block_Adminhtml_Cert_Edit_Tab_History extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        if($model = Mage::registry('giftcert_data')) {
            $id = $model->getId();
        }
        $collection = Mage::getModel('ugiftcert/history')->getCollection();
        if (!is_null($id) && !is_numeric($id)) {
            // there is an ID and it is not an number, then it has to be certificate code
            $collection->addCertCodeFilter($id);
        } else {
            $collection->addCertFilter($id);
        }
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('changelog', array(
            'header'    => $hlp->__('Comments'),
            'align'     => 'left',
            'index'     => 'comments',
        ));
