...
class Unirgy_Giftcert_Block_Adminhtml_Cert_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        if ($id && Mage::getStoreConfig('ugiftcert/email/pdf_enabled')) {
            try {
                $link = new Unirgy_Giftcert_Block_Link(array(
                        'href'  => $this->getUrl('*/*/printout', array('id' => $id))
                                                          ));
                $link->setId('printout');
                $fieldset->addElement($link, '^');
            } catch (Exception $e) {
                // do nothing
                // find way to add link without varien form
            }
        }
        ...
        $fieldset->addField('send_on', 'date', array(
            'name'   => 'send_on',
            'label'  => $hlp->__('Send On'),
            'title'  => $hlp->__('Send On'),
            'image'  => $this->getSkinUrl('images/grid-cal.gif'),
            'format' => Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT),
            'input_format' => Varien_Date::DATE_INTERNAL_FORMAT
        ));

        /*  Adding order id here is not helping with anything so we skip it
                if (!$id) {
        ...
        $fieldset->addField('template_self', 'select', array(
            'name' => 'template_self',
            'label' => $this->__("Gift Certificate Email Template for Self"),
            'title' => $this->__("Gift Certificate Email Template for Self"),
            'values' => Mage::getModel('adminhtml/system_config_source_email_template')->setPath('ugiftcert_email_template_self')->toOptionArray()
        ));

        $fieldset->addField('template', 'select', array(
            'name' => 'template',
            'label' => $this->__("Gift Certificate Email Template for Someone Else"),
            'title' => $this->__("Gift Certificate Email Template for Someone Else"),
            'values' => Mage::getModel('adminhtml/system_config_source_email_template')->setPath('ugiftcert_email_template')->toOptionArray()
        ));

        $fieldset->addField('pdf_template_id', 'select', array(
            'name' => 'pdf_template_id',
            'label' => $this->__("Certificate PDF Template"),
            'title' => $this->__("Certificate PDF Template"),
            'values' => Mage::getModel('ugiftcert/source_pdf')->setPath('ugiftcert/email/pdf_template')->toOptionArray()
        ));
        ...
        $fieldset->addField('disallow_coupons', 'select', array(
             'name'      => 'disallow_coupons',
             'label'     => $hlp->__('Disallow coupons'),
        ...
        if ($cert->getId()) {
            $form->setValues($cert->getData());
        }

        if(isset($link)) {
            $link->setValue($hlp->__('Get printout'));
        }
