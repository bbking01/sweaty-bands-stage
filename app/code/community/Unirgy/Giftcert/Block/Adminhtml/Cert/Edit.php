...
class Unirgy_Giftcert_Block_Adminhtml_Cert_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        if( $model->getId()) {
            $this->_addButton('savesend', array(
                   'label' => Mage::helper('ugiftcert')->__('Save and Send Gift Certificate'),
                   'class' => 'save',
                   'onclick' => 'editForm.submit(\'' . $this->getUrl('*/*/savesend', array($this->_objectId => $model->getId())) . '\');',
              ), 2);
        }
    }
