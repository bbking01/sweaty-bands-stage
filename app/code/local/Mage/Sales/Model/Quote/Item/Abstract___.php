    ...
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
    ...
    public function calcRowTotal()
    {
        ...
        $baseTotal  = $this->getStore()->roundPrice($this->getCalculationPriceOriginal()) * $qty;//$this->getBaseCalculationPriceOriginal() * $qty;
    ...
    public function getCalculationPriceOriginal()
    {
        ...
        if (is_null($price)) {
            ...
            // Code added for add on cost by Rutvi - Starts
		$Item = Mage::getModel('sales/quote_item')->load($this->getData('item_id'));		
		$totalcolor= $Item->getData('totalcolor');
		// Code to fetch add on price from sales_flat_quote_address_item table
		if($totalcolor=='')
		{
		...
		$collection = Mage::getModel('qcprice/qcprice')->getCollection()
																->addFieldToFilter('qr.quantity_range_from',array('lteq'=>array( $this->getTotalQty())))
																->addFieldToFilter('qr.quantity_range_to',array('gteq'=>array( $this->getTotalQty())))
																->addFieldToFilter('clrs.colors_counter',array('eq'=>array($totalcolor)));					
			    												//->addFieldToFilter('clrs.colors_counter',array('lteq'=>array($totalcolor)));	
			...
			$collection->getSelect()
						...
						//->order('clrs.colors_counter DESC');
		if ($collection->count() > 0) {
					$collectionData = $collection->getData();
					$printingPrice = $collectionData[0]['price'];
		}
		$price= $price + $printingPrice ;
		}
		/////////////
            $this->setData('calculation_price', $price);
        }
