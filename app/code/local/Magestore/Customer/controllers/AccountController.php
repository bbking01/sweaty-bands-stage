...
class Magestore_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    /**
	 ...
	 * loginFromToolAction
	 */
	public function loginFromToolAction()
    {
        //if ($this->_getSession()->isLoggedIn()) {
            //$this->_redirect('*/*/');
            //return;
        //}
        $xmlString = '<?xml version="1.0" encoding="iso-8859-1"?><RESPONSE>';
        $session = $this->_getSession();
		$login = $_POST;

		if (!empty($login['email_address']) && !empty($login['password'])) {
			try {
				$session->login($login['email_address'], $login['password']);
				if ($session->getCustomer()->getIsJustConfirmed()) {
                                    $xmlString .= '<STATUS>true</STATUS>';
                                    $xmlString .= '<MESSAGE></MESSAGE>';
                                    $xmlString .= '</RESPONSE>';
                                    echo $xmlString;
                                    exit;
				...
				switch ($e->getCode()) {
					case Mage_Customer_Model_Customer::EXCEPTION_EMAIL_NOT_CONFIRMED:
						$value = Mage::helper('customer')->getEmailConfirmationUrl($login['email_address']);
						$message = Mage::helper('customer')->__('This account is not confirmed. <a href="%s">Click here</a> to resend confirmation email.', $value);
                                                $xmlString .= '<STATUS>false</STATUS>';
                                                $xmlString .= '<MESSAGE>'.$this->__('This account is not confirmed.').'</MESSAGE>';
                                                $xmlString .= '</RESPONSE>';
                                                echo $xmlString;
                                                exit;
					...
					case Mage_Customer_Model_Customer::EXCEPTION_INVALID_EMAIL_OR_PASSWORD:
						$message = $e->getMessage();
						$xmlString .= '<STATUS>false</STATUS>';
                                                $xmlString .= '<MESSAGE>'.$this->__('Invalid Email Or Password.').'</MESSAGE>';
                                                $xmlString .= '</RESPONSE>';
                                                echo $xmlString;
                                                exit;
						break;
					default:
						$message = $e->getMessage();
						$xmlString .= '<STATUS>false</STATUS>';
                                                $xmlString .= '<MESSAGE>'.$e->getMessage().'</MESSAGE>';
                                                $xmlString .= '</RESPONSE>';
                                                echo $xmlString;
                                                exit;
				...
				//$session->addError($message);
				//$session->setUsername($login['email_address']);
			} catch (Exception $e) {
				// Mage::logException($e); // PA DSS violation: this exception log can disclose customer password
			}
		} else {
                        $xmlString .= '<STATUS>false</STATUS>';
                        $xmlString .= '<MESSAGE>'.$this->__('Login and password are required.').'</MESSAGE>';
                        $xmlString .= '</RESPONSE>';
                        echo $xmlString;
                        exit;
			//$session->addError($this->__('Login and password are required.'));
	...
	public function createNewPostAction()
    {
        ...
        if ($session->isLoggedIn()) {
            $xmlString .= '<STATUS>false</STATUS>';
            $xmlString .= '<MESSAGE>Already Login</MESSAGE>';
            $xmlString .= '</RESPONSE>';
            echo $xmlString;
            exit;
        }
		$customerData = array('firstname' => $_POST['firstname'], 'lastname' => $_POST['lastname'], 'email' => $_POST['email']);
        //$session->setEscapeMessages(true); // prevent XSS injection in user input
        if (count($_POST) > 0) {
            $errors = array();
            ...
            try {
                ...
                if ($customerErrors !== true) {
                    ...
                    $xmlString .= '<STATUS>false</STATUS>';
                    $xmlString .= '<MESSAGE>Invalid Data</MESSAGE>';
                    $xmlString .= '</RESPONSE>';
                    echo $xmlString;
                    exit;
                } else {
                    ...
                    $customer->setConfirmation($_POST['confirmation']);
                    $customerErrors = $customer->validate();
                    if (is_array($customerErrors)) {
                        $xmlString .= '<STATUS>false</STATUS>';
                        $xmlString .= '<MESSAGE>Invalid Data</MESSAGE>';
                ...
                if (true === $validationResult) {
                    ...
                    if ($customer->isConfirmationRequired()) {
                        ...
                        $xmlString .= '<STATUS>true</STATUS>';
                        $xmlString .= '<MESSAGE>'.$this->__('Account confirmation is required. Please, check your email for the confirmation link.').'</MESSAGE>';
                        $xmlString .= '</RESPONSE>';
                        echo $xmlString;
                        exit;
                    ...
                    $session->setCustomerFormData($_POST);
                    if (is_array($errors)) {
                        foreach ($errors as $errorMessage) {
                            $session->addError($errorMessage);
                        }
                        ...
                        $xmlString .= '<STATUS>false</STATUS>';
                        $xmlString .= '<MESSAGE>Invalid Customer Data</MESSAGE>';
                        $xmlString .= '</RESPONSE>';
                        echo $xmlString;
                        exit;
                    }
                ...
                if ($e->getCode() === Mage_Customer_Model_Customer::EXCEPTION_EMAIL_EXISTS) {
                    ...
                    //$session->setEscapeMessages(false);
                        ...
                        $xmlString .= '<STATUS>false</STATUS>';
                        $xmlString .= '<MESSAGE>'.$this->__('There is already an account with this email address.').'</MESSAGE>';
                        $xmlString .= '</RESPONSE>';
                        echo $xmlString;
                        exit;
			...
			$xmlString .= '<STATUS>false</STATUS>';
                ...
                //$session->setCustomerFormData($this->getRequest()->getPost())->addException($e, $this->__('Cannot save the customer.'));
                    ...
                    $xmlString .= '<STATUS>false</STATUS>';
                    $xmlString .= '<MESSAGE>'.$this->__('Cannot save the customer.').'</MESSAGE>';
                    $xmlString .= '</RESPONSE>';
                    echo $xmlString;
                    exit;
            }
