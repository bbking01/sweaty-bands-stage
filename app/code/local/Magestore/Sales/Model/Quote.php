...
class Magestore_Sales_Model_Quote extends Mage_Sales_Model_Quote
{
    ...
    {				
        ...
        foreach ($cartCandidates as $candidate) {
			...
			{								
				$dataxml = simplexml_load_string($_REQUEST['dataxml']);	
				$xml = $this->objectsIntoArray($dataxml);
				...
				foreach($nameNumberData as $key => $value)
				{					
					if(count($value['names']) > 0)
					{
						$name[$i] = $value['names'];					
					}
					if(count($value['numbers']) > 0)
					{
						$number[$i] = $value['numbers'];
					}
					$i++;
				}					
			...
			{				
				...
				if(!array_key_exists('0', $orderData['nameData']['data'])){
					$nameNumberData[0] = $orderData['nameData']['data'];
				}
				else
				{
					$nameNumberData = $orderData['nameData']['data'];
				}
