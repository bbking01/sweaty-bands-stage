...
//require_once 'Mage/Sales/Model/Quote/Item/Abstract.php'; 
abstract class Magestore_Sales_Model_Quote_Item_Abstract  extends Mage_Sales_Model_Quote_Item_Abstract
{
    ...
    public function calcRowTotal()
    {
        ...
        $total      = floatval($this->getCalculationPrice1())*$qty;
        $baseTotal  = floatval($this->getBaseCalculationPrice())*$qty;
	...
	public function getCalculationPrice1()
    {
        $price = $this->_getData('calculation_price');
		$xml = simplexml_load_string($_REQUEST['dataxml']);											
		$unitprice = $xml->unitprice;
		
		if (is_null($price)) {          
				
			   //Start - Added For design tool - Naincy
			    ...
			    if($_REQUEST['dataxml'])
			    {
					...
					$this->setData('newtotal', $totalprice);		
			    }
					//End -	Added For design tool - Naincy
				else
				{
					if ($this->hasOriginalCustomPrice()) {
                		$price = $this->getOriginalCustomPrice();
