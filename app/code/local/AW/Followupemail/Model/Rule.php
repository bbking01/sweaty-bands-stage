...
class AW_Followupemail_Model_Rule extends Mage_Core_Model_Abstract
{
    ...
    private function _getCrossProducts($objects)
    {
        ...
        if ($source == AW_Followupemail_Model_Source_Rule_Cross::AW_ARP2) {
            ...
            if (class_exists('AW_Autorelated_Model_Api') && Mage::helper('followupemail')->isARP2Installed()) {
                if ($objects['customer'] == null && $objects['order']) {
                    $objects['customer'] = Mage::getModel('customer/customer')->setData(
                        array(
                            'id'       => $objects['order']->getCustomerId(),
                            'group_id' => $objects['order']->getCustomerGroupId(),
                        )
                    );
                }
    ...
    public function process($params, $objects = array())
    {
        ...
        if ($this->_isValid) {
            ...
            if (
                ...
                $message = 'Email canceled. The customer has unsubscribed from getting this email: customer id=' . $objects['customer_id'] . ' email=' . $objects['customer_email'] . ' rule id=' . $this->getId();
    ...
    */
    public function sendTestEmail($params, $objects = array())
    {
        ...
        $params = $this->_fillTestObjectValues($params);
    ...
    protected function _fillTestObjectValues($params){
        if (empty($params['quote_id'])) {
            $coll = Mage::getModel('sales/quote')->getCollection();
            $coll->getSelect()->order(new Zend_Db_Expr('RAND()'));
            foreach ($coll as $item) {
                $params['quote_id'] = $item->getEntityId();
                break;
            }
        }
        ...
        if (empty($params['customer_id'])) {
            ...
            $coll->getSelect()->order(new Zend_Db_Expr('RAND()'));
            foreach ($coll as $item) {
                $params['customer_id'] = $item->getEntityId();
                break;
            }
        }
        ...
        if (empty($params['product_id'])) {
            ...
            $coll->getSelect()->order(new Zend_Db_Expr('RAND()'));
            foreach ($coll as $item) {
                $params['product_id'] = $item->getEntityId();
                break;
            }
        }
        ...
        if (empty($params['order_increment_id'])) {
            ...
            $coll->getSelect()->order(new Zend_Db_Expr('RAND()'));
            foreach ($coll as $item) {
                $params['order_increment_id'] = $item->getIncrementId();
                break;
            }
        }
