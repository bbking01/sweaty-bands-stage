    ...
    protected function _checkOrderStatusHistory()
    {
        ...
        while ($currentPage <= $_pages) {
            ...
            foreach ($statusHistoryCollection->getItems() as $historyItem) {
                ...
                if ($eventName == AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX . 'new_order') {
                    $_ruleNewCustomer = $dbReader->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW);
                    if (!empty($_ruleNewCustomer)) {
                        $queue->cancelByEvent($order->getCustomerEmail(), AW_Followupemail_Model_Source_Rule_Types::CANCEL_TYPE_CUSTOMER_PLACED_NEW_ORDER, $order->getCustomerId());
                        $queue->cancelByEvent($order->getCustomerEmail(), $eventName, $order->getCustomerId());
                    }
                    $queue->cancelByEvent($order->getCustomerEmail(), AW_Followupemail_Model_Source_Rule_Types::CANCEL_TYPE_CUSTOMER_PLACED_NEW_ORDER, false);
                }
        ...
        foreach ($carts as $cart) {
            ...
            foreach ($ruleIds as $ruleId) {
                ...
                if (sizeof($queueCollection) > 0) {
                    AW_Followupemail_Model_Log::log('dublicated carts abandoned quoteId=' . $cart['quote_id'] . ' , ruleId=' . $ruleId );
                    continue;
                }
