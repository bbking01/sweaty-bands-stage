...
// 'passive' logic version, deletes the emails from the queue by their rules' eventType
    ...
    public function cancelByEvent($email, $eventType, $objectId = false)
    {
        ...
        if ($eventType != AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_PRODUCT_ADD && $objectId !== false) {
        ...
        */
        if ((strpos($eventType, AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX) !== false) && $objectId !== false) {
            $select->where('e.object_id = ?', $objectId);
        }
