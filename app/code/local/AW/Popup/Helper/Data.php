...
class AW_Popup_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    /**
     * Retrive count of popups in the base
     * @return number
     */
    ...
    public function getPopup($pageName)
    {
        ...
        if ($mss = $this->isExtensionAvailable('AW_Marketsuite')) {
        ...
        foreach ($collection as $_item) {
            ...
            if ($_item->getData('mss_rule_id') && $_item->getData('mss_rule_id') != 0 && $mss) {
                $mssRule = Mage::getModel('marketsuite/filter')->load($_item->getData('mss_rule_id'));
                $mssVerification = true;
                if($mssRule->getIsActive()) {
                    $mssVerification = $mssApi->checkRule($customer, $_item->getData('mss_rule_id'));
                }
                if (!$mssVerification) {
                    unset($_item);
                    continue;
        ...
        if (!$popup['width']) {
            if (preg_match('/width=.([0-9]*)/', $popup['popup_content'], $matches))
